...
from KD_Lib.KD.common.loss import MWKDLoss
from KD_Lib.utils import AverageMeter, sharpness, sharpness_gap, log_cfg
from KD_Lib.KD.vision.DML.smooth import Smooth

class MWKD(Smooth):
    def __init__(self, models, loaders, optimizers, schedulers, losses, cfg, logger=None, trial=None):
        super().__init__(models, loaders, optimizers, schedulers, losses, cfg, logger, trial)
        self.loss_smooth = MWKDLoss(self.cfg).to(self.device)
    ...
    def train_student(self, save_model=True):
        ...
        for ep in range(self.cfg.EPOCHS):
            ...
            for (data, label) in self.train_loader:
                ...
                with torch.no_grad():
                    logit_t = self.models[0](data, norm_feats=self.cfg.FEAT_NORM) # Teacher

                logit_s, feat_s, _, _ = self.models[-1](data, return_feats=True, norm_feats=self.cfg.FEAT_NORM) # Student
                ...
                pred_feat_s = self.models[1](feat_s[-1]) # Student KD Head
                ...
                g_sharp, t_sharp, s_sharp = sharpness_gap(logit_t, pred_feat_s)
                s_sharp_train.update(s_sharp), t_sharp_train.update(t_sharp), g_sharp_train.update(g_sharp)
                kld = F.kl_div(F.log_softmax(pred_feat_s.detach(), dim=1), F.softmax(logit_t.detach(), dim=1), reduction='batchmean')
                epoch_kld.update(kld)
                predictions = []
                correct_preds = []
                predictions.append(logit_t.argmax(dim=1, keepdim=True))
            ...
            with torch.no_grad():
                for data, target in self.val_loader:
                    ...
                    if len(model) == 1:
                        ...
                        _, feat_t, _, _ = model[-1](data, return_feats=True)
                        feat_t = [f.detach() for f in feat_t]
                        output = model[0](feat_t[-1]) # Feature selection
    ...
    def set_models(self, mode='eval'):
        if mode == 'eval':
            ...
            [m.train() for m in self.models[:1]], [m.eval() for m in self.models[1:]]
        elif mode == 'train_student':
            [m.eval() for m in self.models[:1]], [m.train() for m in self.models[1:]]
        elif mode == 'train':
